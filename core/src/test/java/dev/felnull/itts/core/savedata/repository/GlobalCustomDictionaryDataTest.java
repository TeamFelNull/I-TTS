...
public class GlobalCustomDictionaryDataTest extends RepoBaseTest {

    @ParameterizedTest
    @MethodSource("globalCustomDictionaryData")
    void testAddAndGetRemove(Map<String, String> dictData, ReplaceType replaceType) {
        DataRepository repo = createRepository();

        // 初期状態でテスト
        CustomDictionaryData customDictionaryData = repo.getGlobalCustomDictionaryData();
        checkAddAndGetRemove(customDictionaryData, dictData, replaceType);

        // キャッシュ参照テスト
        CustomDictionaryData customDictionaryData2 = repo.getGlobalCustomDictionaryData();
        ...
        // 初期状態でテスト
        CustomDictionaryData customDictionaryData3 = repo2.getGlobalCustomDictionaryData();
    ...
    private static Stream<Arguments> globalCustomDictionaryData() {
        return createTestDataStream(Stream.of(customDictionaryData1().toList(), customDictionaryData2().toList()), dictionaryReplaceTypesData())
                .map(data -> {
                    Map<String, String> dictData = new HashMap<>();
                    data.getLeft().forEach(it -> dictData.put(it.getLeft(), it.getRight()));
                    return Arguments.arguments(dictData, data.getRight());
                });
    }
