...
public class OldAudioScheduler extends AudioEventAdapter {
    ...
    //private final AudioLoader audioLoader = new AudioLoader(this);
    ...
    public OldAudioScheduler(AudioPlayer player, BotLocation bag) {
        this.player = player;
    ...
    public void dispose() {
      //  this.audioLoader.dispose();
        ...
        synchronized (previsionLoadTracks) {
            previsionLoadTracks.forEach((k, m) -> {
                m.cancel(true);
            });
            previsionLoadTracks.clear();
        }
    }
    ...
    protected void play(AudioTrack track, float volume) {
